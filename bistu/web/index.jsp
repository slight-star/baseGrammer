...
<body>
    ...
    <a href="${pageContext.request.contextPath }/findMovieByPageServlet?currentPage=1&rows=10">查看电影列表</a>
...
<script>
    $(function() {
        ...
        //显示历年电影数量
        $.post(contextPath+"/findMovieCountServlet",
            // {"year":year},
            function(data){
        ...
        $.post(contextPath+"/findDangqiBoxofficeServlet",
            {"year":year},
            function(data){
                var label_id = "dangqi";
                var dangqi_arr = [];
                ...
                for (var i=0; i<data.length; i++){
                    var data_i = data[i];
                    dangqi_arr.push(data_i["schedule"]); //档期
                    boxoffice_arr.push(data_i["totalBoxoffice"]); //总票房
                }
        ...
        $("#dangqi_year").change(function(){
            ...
            $.post(contextPath+"/findDangqiBoxofficeServlet",
                ...
                function(data){
                    ...
                    for (var i=0; i<data.length; i++){
                        var data_i = data[i];
                        dangqi_arr.push(data_i["schedule"]);
                        boxoffice_arr.push(data_i["totalBoxoffice"]);
                    }
                    barPlot(label_id,"档期总票房(亿)","总票房(亿)",dangqi_arr,boxoffice_arr);
                },"json");
        ...
        $.post(contextPath+"/findMovieTopServlet",
            {"year":year},
            function(data){
                ...
                for (var i=0; i<data.length; i++){
                    var data_i = data[i];
                    movieName_arr.push(data_i["title"]);
                    boxoffice_arr.push(data_i["totalBoxoffice"]);
                }
        ...
        $(document).ready(function () {
            $.post(contextPath+"/findTypeTopServlet",
                function (data) {
